...
check_kit <- function(kit){
  ...
  if(all(is.na(kit_df))){
    stop('Kit ', kit, ' does not exist in euroformix. Use `euroformix::getKit(kit)` for a list of preexisting kits in euroformix and the vignette included in this package for how to include a custom kit.')
  }else{
    return(kit_df)
  }
}
