...
#'
#' @description Format population allele frequencies for
#' input into the euroformix `contLikSearch()` function.
#'
#' @inheritParams bistro
#'
#' @return A list of allele frequencies for the population in the format
#'   required for input into the euroformix `contLikSearch()` function
#' @keywords internal
format_allele_freqs <- function(pop_allele_freqs) {
  allele_names <- pop_allele_freqs$Allele
  marker_names <-
    names(pop_allele_freqs)[names(pop_allele_freqs) != "Allele"]
...
#'
#' @description Format human STR profiles for input into
#' the euroformix `contLikSearch()` function.
#'
#' @inheritParams bistro
#'
#' @return A list of lists including the alleles for each sample in the format
#'   required for input into the euroformix `contLikSearch()` function
#' @keywords internal
...
#'
#' @description Format bloodmeal STR profiles for input into the euroformix
#'   `contLikSearch()` function.
#'
#' @inheritParams bistro
#'
#' @return A list of lists including the alleles for each sample in the format
