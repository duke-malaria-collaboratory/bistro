# global variables
utils::globalVariables(c("Marker", "Allele", "allele_count", "locus_n", "freq", "SampleName", "Height", "peaks", "index", "Allele1", "Allele2", "all_alleles", "n", "log10LR", "note", "human_id", "thresh_low", "next_same", "n_samps"))
...
rm_dups <- function(df){
  ...
  if(n_orig != dplyr::n_distinct(df)){
    df <- df |>
      dplyr::distinct()
    warning(paste0("Detected and removed ", n_orig - nrow(df), " duplicate rows."))
  }
  return(df)
}
...
rm_twins <- function(human_profiles){
  not_twins <- human_profiles |>
    ...
    dplyr::summarize(all_alleles = stringr::str_c(paste0(Marker, Allele), collapse = ';')) |>
    dplyr::group_by(all_alleles) |>
    dplyr::mutate(n = dplyr::n()) |>
    dplyr::filter(n == 1) |>
    dplyr::pull(SampleName)

  n_ident_profs <- dplyr::n_distinct(human_profiles$SampleName) - length(not_twins)

  if(n_ident_profs > 0){
    message(paste0('Identified ', n_ident_profs, ' people whose profiles appear more than once (likely identical twins). These are being removed.'))
...
subset_ids <- function(ids, vec, vec_name){
  ...
  if(length(id_absent) > 0){
    warning("Removing ", length(id_absent), " ", vec_name, " not found in the dataset: ", id_absent)
  }
