test_that("check_bistro_input works", {
  expect_no_error(check_bistro_inputs(bloodmeal_profiles, human_profiles, 'ESX17', 1, NULL, TRUE))
})

test_that("check_is_bool works", {
  expect_no_error(check_is_bool(TRUE, 'v1'))
  expect_error(check_is_bool(1, 'v1'),
...
test_that("check_is_numeric works", {
  ...
  expect_error(check_is_numeric(0, 'v1', pos = TRUE),
               'v1 must be greater than zero, but is 0.')
  expect_error(check_is_numeric('a', 'v1'),
               'v1 must be numeric, but is character.')
})

test_that("check_kit works", {
  expect_error(check_kit('nokit'),
               'Kit nokit does not exist in euroformix.')
  expect_no_error(expect_equal(colnames(check_kit('ESX17')), c("Panel", "Short.Name", "Full.Name", "Marker", "Allele", "Size", "Size.Min", "Size.Max", "Virtual", "Color", "Repeat", "Marker.Min", "Marker.Max", "Offset", "Gender.Marker")))
})
...
test_that('check_calc_allele_freqs works', {
  ...
  expect_error(check_calc_allele_freqs(1),
...
test_that('check_if_allele_freqs works', {
  ...
  expect_warning(check_if_allele_freqs(pop_allele_freqs, FALSE, euroformix::getKit('ESX17')),
                 '1/17 markers in kit but not in pop_allele_freqs: AMEL')
  expect_error(check_if_allele_freqs(NULL, FALSE),
...
test_that("check_ids works", {
  ...
  expect_error(check_ids(tibble::tibble(test=numeric()), 'v1'),
...
test_that("check_colnames works", {
  ...
  expect_error(check_colnames(tibble::tibble(test=1), 'test1'),
               "Not all expected column names are present. Missing: test1")

})

test_that("check_threshT works", {

  expect_no_error(check_threshT(1))

  expect_error(check_threshT(-1),
               "thresT must be \u2265 0, but is -1")

  expect_error(check_threshT('test'),
               "thresT must be numeric, but is character")

})

test_that("check_setdiff_markers works", {
  expect_warning(expect_warning(check_setdiff_markers(1:3, c(1, 4), 'm1', 'm2'),
