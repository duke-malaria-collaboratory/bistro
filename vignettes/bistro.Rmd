...
Next, there is a message notifying you that log10_lrs (log10 likelihood ratios) are being calculated for each bloodmeal-human pair (for 4 bloodmeals and 3 humans). This can take a while, so the progress is also printed out. 
...
- `thresh_low`: log10_lr threshold at which the match was made
...
- all log10_lrs < 1.5: none of the log10_lrs were high enough to be considered a match. 
...
- `model_degrad`: whether or not to model peak degradation when calculating log-likelihoods using `euroformix::contLikSearch()` (default: TRUE)
- `model_bw_stutt` and `model_fw_stutt`: whether or not to model peak backward and forward stutter when calculating log-likelihoods using `euroformix::contLikSearch()` (default: FALSE)
